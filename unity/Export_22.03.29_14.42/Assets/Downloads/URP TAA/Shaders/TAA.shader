Shader "Hidden/TAA"
{
    ...
    #define ZCMP_GT(a, b) (a > b)
        ...
        float4 Frag(VaryingsTAA input) : SV_Target
        {
        ...
        #if _TAA_UseDilation
            float2 mv = SAMPLE_TEXTURE2D_X(_MotionVectorTexture, sampler_PointClamp, find_closest_uv(depth, uv)).rg;
        #else
            float2 mv = SAMPLE_TEXTURE2D_X(_MotionVectorTexture, sampler_PointClamp, uv).rg;
        #endif
            float2 prev_uv = uv - mv - _TAA_Offset.xy;
            mv = uv - prev_uv;
        #else
            ...
            if (prev_uv.x > 1 || prev_uv.y > 1 || prev_uv.x < 0 || prev_uv.y < 0)
            {
                return float4(filter(samples), 1.0);
            }
        ...
        #endif
        #endif
        float final_blend = lerp(_TAA_Blend, 0.2, saturate(length(mv) * 20));
        #if _TAA_UseTonemap
